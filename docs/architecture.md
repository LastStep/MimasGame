...
| `UI/Ramps` | The fades USS cannot draw — the floors, the band, the plate's wash — as small white-alpha textures tinted by a token, and the lineage painting; generated once, cached |
| `UI/FontSpacing` | Clears TextCore's "ignore spacing" flag on the fonts' kerning pairs at bind, so letter-spacing applies between every pair of capitals |
